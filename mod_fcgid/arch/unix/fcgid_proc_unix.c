								...
								fcgid_procnode* procnode)
{
	...
	*/
    ...
    apr_snprintf(unix_addr.sun_path, sizeof(unix_addr.sun_path)-1,
		"%s/%d.%d", 
		g_socket_dir, getpid(), g_process_counter++);
	...
	if( !(procnode->proc_id=apr_pcalloc(procnode->proc_pool, sizeof(apr_proc_t)))
		|| (rv=apr_procattr_create(&procattr, procnode->proc_pool))!=APR_SUCCESS
		|| (rv=apr_procattr_child_err_set(procattr, procinfo->main_server->error_log, 
							NULL))!=APR_SUCCESS
		...
		|| (rv=apr_procattr_child_in_set(procattr, file, NULL))!=APR_SUCCESS )
	{
		ap_log_error(APLOG_MARK, APLOG_ERR, rv, procinfo->main_server,
			"mod_fcgid: couldn't set child process attributes: %s", unix_addr.sun_path);
		close(unix_socket);
		return rv;
	}
...
{
	fcgid_namedpipe_handle* handle_info = (fcgid_namedpipe_handle*)thesocket;
							...
							fcgid_ipc* ipc_handle)
{
	...
	apr_pool_cleanup_register(ipc_handle->request_pool, 
				handle_info, ipc_handle_cleanup, apr_pool_cleanup_null);

	/* Connect to fastcgi server */
    memset(&unix_addr, 0, sizeof(unix_addr));
    unix_addr.sun_family = AF_UNIX;
    strncpy(unix_addr.sun_path, procnode->socket_path, sizeof(unix_addr.sun_path)-1);
...
static apr_status_t read_fcgi_header(server_rec* main_server,
							 fcgid_ipc* ipc_handle,
							 FCGI_Header* header)
{
	fcgid_namedpipe_handle* handle_info = (fcgid_namedpipe_handle*)ipc_handle->ipc_handle_info;
	fd_set rset;
	struct timeval tv;
...
static apr_status_t handle_fcgi_body(server_rec* main_server,
						fcgid_ipc* ipc_handle,
						FCGI_Header* header,
							...
							apr_bucket_alloc_t* alloc)
{
	...
	*/
		...
		while( has_write<write_buf_len )
		{
			...
			if( FD_ISSET(unix_socket, &rset) )
			{
				if( read_fcgi_header(main_server, 
							ipc_handle, &fcgi_header)!=APR_SUCCESS
			...
			if( FD_ISSET(unix_socket, &wset) )
			{
				...
				{
					ap_log_error(APLOG_MARK, APLOG_WARNING, apr_get_os_error(), 
						main_server, "mod_fcgid: write error on unix socket");
					return APR_ESPIPE;
				}
				has_write += retcode;
			}
	...
	while( 1 )
	{
		...
		{
			if( read_fcgi_header(main_server, ipc_handle, &fcgi_header)!=APR_SUCCESS
						  ...
						  apr_exit_why_e exitwhy, server_rec* main_server)
{
	...
	apr_snprintf(key_name, _POSIX_PATH_MAX, "%lX%lX", 
		procnode->inode, procnode->deviceid);
	apr_pool_userdata_get((void**)&cgipath, key_name, g_inode_cginame_map);
    ...
    if( APR_PROC_CHECK_SIGNALED(exitwhy) )
	{
		switch( signum ) 
		{
			...
			default:
				...
				{
					apr_snprintf(signal_info, HUGE_STRING_LEN-1,
						"get signal %d, possible coredump generated", signum);
				}
				else
				{
					apr_snprintf(signal_info, HUGE_STRING_LEN-1,
	...
	{
		apr_snprintf(signal_info, HUGE_STRING_LEN-1,
			"terminated by calling exit(), return code: %d", exitcode);
	}
