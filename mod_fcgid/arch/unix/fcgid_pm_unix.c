...
{
	apr_status_t rv;
	g_process_manager = (apr_proc_t*)apr_pcalloc(configpool, sizeof(*g_process_manager));
	...
	apr_pool_note_subprocess(configpool, g_process_manager, APR_KILL_ONLY_ONCE);
...
apr_status_t procmgr_child_init(server_rec *main_server, apr_pool_t* configpool)
{
	/* Do nothing on *NIX platform */
	return APR_SUCCESS;
}

apr_status_t procmgr_post_config(server_rec *main_server, apr_pool_t* configpool)
{
	apr_status_t rv;
	...
	if( (rv=apr_dir_make_recursive(get_socketpath(main_server),
		APR_UREAD|APR_UWRITE|APR_UEXECUTE, configpool))!=APR_SUCCESS
		...
		*/
		if( g_main_server )
			ap_log_error(APLOG_MARK, LOG_WARNING, rv, g_main_server,
				"mod_fcgid: can't write to pipe, write byte: %d", nbytes);
		return rv;
	}
