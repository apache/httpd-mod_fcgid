...
apr_status_t proctable_post_config(server_rec *main_server, apr_pool_t* configpool)
{
				...
				main_server->process->pconf))!=APR_SUCCESS )
	{
		ap_log_error(APLOG_MARK, APLOG_EMERG, rv, main_server,
					"mod_fcgid: Can't create share memory for size %ld byte", shmem_size);
		exit(1);
    }
	...
	/* Create global mutex */
    if( (rv=apr_global_mutex_create(&g_sharelock, tmpnam(g_sharelock_name),
                    APR_LOCK_DEFAULT, main_server->process->pconf))!=APR_SUCCESS )
	{
		ap_log_error(APLOG_MARK, APLOG_EMERG, rv, main_server,
...
apr_status_t proctable_child_init(server_rec *main_server, apr_pool_t* configpool)
{
	apr_status_t rv;
	if( (rv=apr_global_mutex_child_init(&g_sharelock, 
					g_sharelock_name, main_server->process->pconf))!=APR_SUCCESS )
	{
		ap_log_error(APLOG_MARK, APLOG_EMERG, rv, main_server,
