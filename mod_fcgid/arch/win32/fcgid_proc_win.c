								...
								fcgid_procnode* procnode)
{
	...
	if( !g_inode_cginame_map )
		apr_pool_create(&g_inode_cginame_map, procinfo->main_server->process->pconf);
	if( !g_inode_cginame_map )
	{
	...
	*finish_event = CreateEvent(NULL, TRUE, FALSE, NULL);
	...
	listen_handle = CreateNamedPipe(sock_path, 
			...
			PIPE_UNLIMITED_INSTANCES, 8192, 8192, 0, &SecurityAttributes);
	...
	/* Create process now */
	if( !(procnode->proc_id=apr_pcalloc(procnode->proc_pool, sizeof(apr_proc_t)))
		|| (rv=apr_procattr_create(&proc_attr, procnode->proc_pool))!=APR_SUCCESS
		|| (rv=apr_procattr_dir_set(proc_attr, 
					ap_make_dirstr_parent(procnode->proc_pool, 
					procinfo->cgipath)))!=APR_SUCCESS
		|| (rv=apr_procattr_cmdtype_set(proc_attr, APR_PROGRAM))!=APR_SUCCESS
		|| (rv=apr_procattr_detach_set(proc_attr, 1))!=APR_SUCCESS
		|| (rv=apr_os_file_put(&file, &listen_handle, 0, 
					procnode->proc_pool))!=APR_SUCCESS
		|| (rv=apr_procattr_child_in_set(proc_attr, file, NULL))!=APR_SUCCESS
		|| (rv=apr_proc_create(procnode->proc_id, procinfo->cgipath, NULL, 
					proc_environ, proc_attr, procnode->proc_pool))!=APR_SUCCESS )
	{
							...
							fcgid_ipc* ipc_handle)
{
	...
	ipc_handle->ipc_handle_info = (fcgid_namedpipe_handle *)apr_pcalloc(ipc_handle->request_pool, 
		...
		&& GetLastError()==ERROR_PIPE_BUSY)
	{
		...
		if( WaitNamedPipe(procnode->socket_path, 
							ipc_handle->connect_timeout) )
		{
			handle_info->handle_pipe = CreateFile(procnode->socket_path,
						GENERIC_READ|GENERIC_WRITE, 
						0, /* no sharing */
	...
	if( handle_info->handle_pipe==INVALID_HANDLE_VALUE )
	{
		if( GetLastError()==ERROR_FILE_NOT_FOUND ) /* The process has exited */
			...
			ap_log_error(APLOG_MARK, APLOG_DEBUG, apr_get_os_error(), main_server,
...
static BOOL read_fcgi_header(server_rec* main_server,
							 fcgid_ipc* ipc_handle,
							 FCGI_Header* header)
{
	...
	while( has_read<sizeof(*header) )
	{
		...
		else if( (rv=GetLastError())!=ERROR_IO_PENDING )
		{
			ap_log_error(APLOG_MARK, APLOG_WARNING, rv, 
					main_server, "mod_fcgid: can't read header from pipe");
			return FALSE;	
		}
		else
		{
			...
			else
			{
				ap_log_error(APLOG_MARK, APLOG_WARNING, 0, 
						main_server, "mod_fcgid: read header timeout from pipe");
				return FALSE;
			}
		}
	}
...
static BOOL handle_fcgi_body(server_rec* main_server,
						fcgid_ipc* ipc_handle,
						FCGI_Header* header,
						...
						apr_bucket_alloc_t* alloc)
{
		...
		|| header->type==FCGI_END_REQUEST )
	{
		...
		{
			ap_log_error(APLOG_MARK, APLOG_WARNING, apr_get_os_error(), 
					main_server, "mod_fcgid: can't alloc memory for respond: %d",
					readsize+1);
			return FALSE;
		}
		...
		while( has_read<readsize )
		{
			DWORD bytesread;
			if( ReadFile(handle_info->handle_pipe, readbuf+has_read, 
					readsize-has_read, &bytesread, &handle_info->overlap_read) )
			{
				has_read += bytesread;
				continue;
			}
		...
		if( header->type==FCGI_STDERR )
		{
			...
			content_len += header->contentLengthB0;
				...
				|| ( content_len==2 && readbuf[0]=='\r' && readbuf[1]=='\n' )
				|| content_len==0) )
			{
				ap_log_error(APLOG_MARK, APLOG_WARNING, apr_get_os_error(), 
						main_server, "mod_fcgid: cgi stderr log: %s", readbuf);
			}
		...
		else if( header->type==FCGI_STDOUT )
		{
			...
			apr_bucket* bucket_stdout = apr_bucket_heap_create(readbuf, 
					readsize, apr_bucket_free,
					alloc);
			if( !bucket_stdout )
			{
				apr_bucket_free(readbuf);
				ap_log_error(APLOG_MARK, APLOG_WARNING, apr_get_os_error(), 
					main_server, "mod_fcgid: can't alloc memory for stdout bucket");
				return FALSE;
			}
	...
	ap_log_error(APLOG_MARK, APLOG_WARNING, 0, 
			main_server, "mod_fcgid: invalid respond type: %d", header->type);
	return FALSE;
}
							...
							apr_bucket_alloc_t* alloc)
{
	...
	*/
	...
	APR_BRIGADE_FOREACH(bucket_request, birgade_send)
	{
		...
		{
			ap_log_error(APLOG_MARK, APLOG_WARNING, rv, 
					main_server, "mod_fcgid: can't read request from bucket");
			return rv;
		}
			...
			else
			{
				...
				*/
				...
				while( PeekNamedPipe(handle_info->handle_pipe, &fcgi_header,
						sizeof(fcgi_header), &BytesRead, NULL, NULL)
						&& BytesRead==sizeof(fcgi_header) )
				{
				...
				{
					ap_log_error(APLOG_MARK, APLOG_WARNING, 0, 
							main_server, "mod_fcgid: write timeout to pipe");
					return APR_ESPIPE;
				}
			}
		}
...
apr_status_t proc_close_ipc(fcgid_ipc* ipc_handle)
{
	return apr_pool_cleanup_run(ipc_handle->request_pool, 
		ipc_handle->ipc_handle_info, ipc_handle_cleanup);
}
						  ...
						  apr_exit_why_e exitwhy, server_rec* main_server)
{
	...
	if( cgipath )
		...
		ap_log_error(APLOG_MARK, APLOG_WARNING, 0, main_server,
			"mod_fcgid: can't get cgi name while exiting, exitcode: %d", exitcode);
}
