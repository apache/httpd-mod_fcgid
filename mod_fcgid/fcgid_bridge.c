										   ...
										   apr_ino_t inode, apr_dev_t deviceid)
{
	...
	while( current_node!=proc_table )
	{
		...
		if( current_node->inode==inode && current_node->deviceid==deviceid )
		{
...
static void return_procnode(server_rec* main_server, 
...
static int bridge_request_once(request_rec* r, const char* argv0, 
	...
	*/
	...
	if( !brigade_stdout )
	{
		ap_log_error(APLOG_MARK, APLOG_WARNING, apr_get_os_error(), 
			r->server, "mod_fcgid: can't alloc memory for stdout brigade");
	...
	else if( (rv=proc_bridge_request(r->server, &ipc_handle, output_brigade,
				brigade_stdout, r->connection->bucket_alloc))!=APR_SUCCESS )
	{
	...
	if( apr_time_sec(apr_time_now())-apr_time_sec(begin_request_time)>g_busy_timeout )
	{
		/* I have to return and do nothing to the process slot */
		ap_log_error(APLOG_MARK, APLOG_WARNING, apr_get_os_error(), 
			r->server, "mod_fcgid: process busy timeout, take %d seconds for this request",
			apr_time_sec(apr_time_now())-apr_time_sec(procnode->last_active_time));
	...
	if( !communicate_error )
	{
		...
		if( ap_scan_script_header_err_brigade(r, brigade_stdout, sbuf)!=OK )
		{
			ap_log_error(APLOG_MARK, APLOG_WARNING, 0, 
					r->server, "mod_fcgid: invalid script header");
			apr_brigade_destroy(brigade_stdout);
			return HTTP_INTERNAL_SERVER_ERROR;
		}
		...
		if( (rv=ap_pass_brigade(r->output_filters, brigade_stdout))!=APR_SUCCESS )
		{
			ap_log_error(APLOG_MARK, APLOG_WARNING, rv, 
					r->server, "mod_fcgid: can't pass the respond to output filter");
		}
	}
...
int bridge_request(request_rec* r, const char* argv0)
{
	...
	output_brigade = apr_brigade_create(request_pool, r->connection->bucket_alloc);
	if( !output_brigade )
	{
		ap_log_error(APLOG_MARK, APLOG_WARNING, apr_get_os_error(), 
			main_server, "mod_fcgid: can't alloc memory for output brigade");
		return HTTP_INTERNAL_SERVER_ERROR;
	}

	/* Build the begin request and environ request, append them to output_brigade */
	if( !build_begin_block(r->server, r->connection->bucket_alloc, output_brigade)
		|| !build_env_block(r->server, envp, 
				r->connection->bucket_alloc, output_brigade) )
	{
		ap_log_error(APLOG_MARK, APLOG_WARNING, 0, 
			main_server, "mod_fcgid: can't build begin or env request");
	...
	*/
	...
	do
	{
						...
						APR_BLOCK_READ, HUGE_STRING_LEN))!=APR_SUCCESS )
		{
			ap_log_error(APLOG_MARK, APLOG_INFO, 0, 
				main_server, "mod_fcgid: can't get data from http client");
			apr_brigade_destroy(output_brigade);
			if( input_brigade )
        ...
        APR_BRIGADE_FOREACH(bucket_input, input_brigade)
		{
			...
			{
				ap_log_error(APLOG_MARK, APLOG_WARNING, rv, 
					main_server, "mod_fcgid: can't read request from HTTP client");
				apr_brigade_destroy(input_brigade);
				apr_brigade_destroy(output_brigade);
				return HTTP_INTERNAL_SERVER_ERROR;
			}
			...
			stdin_request_header = apr_bucket_alloc(sizeof(FCGI_Header),
						r->connection->bucket_alloc);
			bucket_header = apr_bucket_heap_create((const char*)stdin_request_header, 
						sizeof(*stdin_request_header), apr_bucket_free,
						r->connection->bucket_alloc);
			apr_bucket_copy(bucket_input, &bucket_stdin);
			if( !stdin_request_header || !bucket_header || !bucket_stdin
				|| !init_header(FCGI_STDIN, 1, len, 0, stdin_request_header) )
			{
				ap_log_error(APLOG_MARK, APLOG_WARNING, apr_get_os_error(), 
					main_server, "mod_fcgid: can't alloc memory for stdin request");
				apr_brigade_destroy(input_brigade);
				apr_brigade_destroy(output_brigade);
				return HTTP_INTERNAL_SERVER_ERROR;
			}
	...
	{
		ap_log_error(APLOG_MARK, APLOG_WARNING, apr_get_os_error(), 
			main_server, "mod_fcgid: can't alloc memory for eos bucket");
		apr_brigade_destroy(output_brigade);
		return HTTP_INTERNAL_SERVER_ERROR;
	}
	...
	for( i=0; i<FCGID_REQUEST_COUNT; i++)
	{
		...
		if( (retcode=bridge_request_once(r, argv0, output_brigade))==HTTP_OK )
			break;
