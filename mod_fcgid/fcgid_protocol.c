...
static void init_begin_request_body(int role, FCGI_BeginRequestBody* begin_request_body)
{
...
int build_begin_block(server_rec* main_server, 
		apr_bucket_alloc_t* alloc, apr_bucket_brigade* request_brigade)
{
	/* Alloc memory for begin request header & body */
	FCGI_Header* begin_request_header = apr_bucket_alloc(sizeof(FCGI_Header), alloc);
	FCGI_BeginRequestBody* begin_request_body = apr_bucket_alloc(
						sizeof(FCGI_BeginRequestBody), alloc);
	apr_bucket* bucket_header = apr_bucket_heap_create(
						(const char*)begin_request_header, 
						sizeof(*begin_request_header), apr_bucket_free, alloc);
	apr_bucket* bucket_body = apr_bucket_heap_create(
						(const char*)begin_request_body, 
						sizeof(*begin_request_body), apr_bucket_free, alloc);
		...
		|| !bucket_header || !bucket_body )
	{
		ap_log_error(APLOG_MARK, APLOG_WARNING, apr_get_os_error(), 
			main_server, "mod_fcgid: can't alloc memeory for begin request");
		return 0;		
	}
	...
	{
		ap_log_error(APLOG_MARK, APLOG_WARNING, apr_get_os_error(), 
			main_server, "mod_fcgid: can't init begin request header");
		return 0;		
	}
		...
		apr_bucket_alloc_t* alloc, apr_bucket_brigade* request_brigade)
{
	...
	FCGI_Header* env_request_header = apr_bucket_alloc(sizeof(FCGI_Header), alloc);
	FCGI_Header* env_empty_header = apr_bucket_alloc(sizeof(FCGI_Header), alloc);
	char* buf = apr_bucket_alloc(bufsize, alloc);
	apr_bucket* bucket_header = apr_bucket_heap_create(
						(const char*)env_request_header, 
						sizeof(*env_request_header), apr_bucket_free, alloc);
	apr_bucket* bucket_body = apr_bucket_heap_create(buf, bufsize, 
						apr_bucket_free, alloc);
	apr_bucket* bucket_empty_header = apr_bucket_heap_create(
						(const char*)env_empty_header,
						sizeof(*env_empty_header), apr_bucket_free, alloc);
	if( !env_request_header || !buf || !env_empty_header
		|| !bucket_header || !bucket_body || !bucket_empty_header )
	{
		ap_log_error(APLOG_MARK, APLOG_WARNING, apr_get_os_error(), 
			main_server, "mod_fcgid: can't alloc memory for envion");
		return 0;		
	}
	...
	{
		ap_log_error(APLOG_MARK, APLOG_WARNING, apr_get_os_error(), 
			main_server, "mod_fcgid: can't init env request header");
		return 0;
	}
