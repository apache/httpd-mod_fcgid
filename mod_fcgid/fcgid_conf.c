...
void* create_fcgid_config(apr_pool_t* p, server_rec* s)
{
    ...
    config->sockname_prefix = ap_server_root_relative(p, DEFAULT_SOCKET_PREFIX);
...
const char* set_idle_scan_interval(cmd_parms* cmd, void* dummy, const char* arg)
{
    server_rec* s = cmd->server; 
    fcgid_conf* config = ap_get_module_config(s->module_config, &fcgid_module);
...
const char* set_busy_scan_interval(cmd_parms* cmd, void* dummy, const char* arg)
{
    server_rec* s = cmd->server; 
    fcgid_conf* config = ap_get_module_config(s->module_config, &fcgid_module);
...
const char* set_error_scan_interval(cmd_parms* cmd, void* dummy, const char* arg)
{
    server_rec* s = cmd->server; 
    fcgid_conf* config = ap_get_module_config(s->module_config, &fcgid_module);
...
const char* set_zombie_scan_interval(cmd_parms* cmd, void* dummy, const char* arg)
{
    server_rec* s = cmd->server; 
    fcgid_conf* config = ap_get_module_config(s->module_config, &fcgid_module);
...
const char* set_spawnscore_uplimit(cmd_parms* cmd, void* dummy, const char* arg)
{
    server_rec* s = cmd->server; 
    fcgid_conf* config = ap_get_module_config(s->module_config, &fcgid_module);
...
const char* set_termination_score(cmd_parms* cmd, void* dummy, const char* arg)
{
    server_rec* s = cmd->server; 
    fcgid_conf* config = ap_get_module_config(s->module_config, &fcgid_module);
...
const char* set_default_max_class_process(cmd_parms* cmd, void* dummy, const char* arg)
{
    server_rec* s = cmd->server; 
    fcgid_conf* config = ap_get_module_config(s->module_config, &fcgid_module);
...
{
    fcgid_conf* config = ap_get_module_config(s->module_config, &fcgid_module);
    return config? config->default_max_class_process_count:DEFAULT_MAX_CLASS_PROCESS_COUNT;
}

const char* set_ipc_connect_timeout(cmd_parms* cmd, void* dummy, const char* arg)
{
    server_rec* s = cmd->server; 
    fcgid_conf* config = ap_get_module_config(s->module_config, &fcgid_module);
...
const char* set_ipc_comm_timeout(cmd_parms* cmd, void* dummy, const char* arg)
{
    server_rec* s = cmd->server; 
    fcgid_conf* config = ap_get_module_config(s->module_config, &fcgid_module);
...
const char* set_server_config(cmd_parms* cmd, void* dummy, const char* thearg)
{
	...
	serverinfo->init_env 
		= apr_table_make(cmd->server->process->pconf,10);
    ...
    while( 1 )
	{
		...
		if( apr_strnatcasecmp(arg, "-initenv")==0 )
		{
			...
			if( *key )
			{
				...
				apr_table_set(serverinfo->init_env,
							key, value?value:"");
			}
		}
					 ...
					 struct fcgi_server_info* info)
{
	...
	{
		if( matchnode->inode==inode 
			&& matchnode->deviceid==deviceid)
			break;
	}
	...
	else
	{
		...
		*/
		if( !matchnode->has_merge )
		{
			...
			const apr_array_header_t *barr = apr_table_elts(config->default_init_env);
			...
			for (i = 0; i < barr->nelts; ++i) 
			{
				...
				if( !apr_table_get(matchnode->init_env, belt[i].key) )
					apr_table_set(matchnode->init_env, 
								belt[i].key, belt[i].val);
			}
