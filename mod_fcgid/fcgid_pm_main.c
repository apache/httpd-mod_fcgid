...
static void
scan_idlelist (server_rec * main_server)
{
  /* 
     Scan the idle list 
...
static void
scan_busylist (server_rec * main_server)
{
  /*
     Scan the busy list
...
static void
scan_idlelist_zombie (server_rec * main_server)
{
  /* 
     Scan the idle list 
  ...
  while (current_node != proc_table)
      ...
      if (proc_wait_process (main_server, current_node) != APR_CHILD_NOTDONE)
	{
	  ap_log_error (APLOG_MARK, APLOG_WARNING, 0, main_server,
			"mod_fcgid: cleanup zombie process %" APR_PID_T_FMT,
			thepid);
...
static void
scan_errorlist (server_rec * main_server)
{
  /* 
...
static void
kill_all_subprocess (server_rec * main_server)
{
  size_t i;
  int exitcode;
...
apr_status_t
pm_main (server_rec * main_server, apr_pool_t * configpool)
{
