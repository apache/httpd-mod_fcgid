...
static link_node_to_list(server_rec* main_server, 
						 fcgid_procnode* header, 
						 fcgid_procnode* node,
						 fcgid_procnode* table_array)
{
...
static void scan_idlelist(server_rec *main_server)
{
	...
	*/
	...
	if( procmgr_must_exit() 
		|| apr_time_sec(now)-apr_time_sec(lastidlescan)<=g_idle_scan_interval )
		return;
	lastidlescan = now;
	...
	while( current_node!=proc_table )
	{
		...
		if( apr_time_sec(now)-apr_time_sec(last_active_time)>g_idle_timeout 
			|| apr_time_sec(now)-apr_time_sec(start_time)>g_proc_lifetime )
		{
			/* Set die reason for log */
			if( apr_time_sec(now)-apr_time_sec(last_active_time)>g_idle_timeout )
				current_node->diewhy = FCGID_DIE_IDLE_TIMEOUT;
			else if( apr_time_sec(now)-apr_time_sec(start_time)>g_proc_lifetime )
...
static void scan_busylist(server_rec *main_server)
{
	...
	*/
	...
	if( procmgr_must_exit() 
		|| apr_time_sec(now)-apr_time_sec(lastbusyscan)<=g_busy_scan_interval )
		return;
	lastbusyscan = now;
	...
	while( current_node!=proc_table )
	{
		...
		last_active_time = current_node->last_active_time;
		if( apr_time_sec(now)-apr_time_sec(last_active_time)>g_busy_timeout )
		{
...
static void scan_idlelist_zombie(server_rec* main_server)
{
	...
	*/
	...
	if( procmgr_must_exit() 
		|| apr_time_sec(now)-apr_time_sec(lastzombiescan)<=g_zombie_scan_interval )
		return;
	lastzombiescan = now;
	...
	*/
	...
	while( current_node!=proc_table )
	{
		...
		last_active_time = current_node->last_active_time;
		if( apr_time_sec(now)-apr_time_sec(last_active_time)>g_zombie_scan_interval )
		{
			/* Unlink from idle list */
			previous_node->next_index = current_node->next_index;
	...
	while( current_node!=proc_table )
	{
								...
								!=APR_CHILD_NOTDONE )
		{
			ap_log_error(APLOG_MARK, APLOG_WARNING, 0, main_server,
				"mod_fcgid: cleanup zombie process %"APR_PID_T_FMT,	thepid);
...
static void scan_errorlist(server_rec *main_server)
{
	...
	*/
	...
	/* Should I check the busy list? */
	if( procmgr_must_exit() 
		|| apr_time_sec(now)-apr_time_sec(lasterrorscan)<=g_error_scan_interval )
		return;
	...
	while( current_node!=proc_table )
	{
		...
		if( proc_wait_process(main_server,
							current_node)!=APR_CHILD_NOTDONE )
		{
			...
			current_node=&proc_table[current_node->next_index] )
	{
		...
		{
			ap_log_error(APLOG_MARK, APLOG_WARNING, 0, main_server,
				"mod_fcgid: process %"APR_PID_T_FMT" graceful kill fail, sending SIGKILL",
	...
	for( i=0; i<proctable_get_table_size(); i++ )
	{
		if( proc_table[i].proc_pool )
		{
						...
						APR_NOWAIT)!=APR_CHILD_NOTDONE )
			{
				...
				proc_print_exit_info(&proc_table[i], exitcode, exitwhy, main_server);
						  ...
						  apr_pool_t* configpool)
{
	...
	else
	{
		...
		ap_log_error(APLOG_MARK, APLOG_NOTICE, 0, main_server,
				"mod_fcgid: server %s(%" APR_PID_T_FMT ") started", command->cgipath,
...
apr_status_t pm_main(server_rec *main_server, apr_pool_t* configpool)
{
	...
	while( 1 )
	{
		...
		/* Wait for command */
		if( procmgr_peek_cmd(&command)==APR_SUCCESS
			&& is_spawn_allowed(main_server, &command) )
